...
We will use genotyping data from the 1000 genomes project. Specifically we will use genotypes from
Yoruba individuals from Nigeria

### Read Samples

Read in the identifiers of the samples. Each sample is an immortalized cell line derived from a different Yoruba individual. 
...
sum(genotype.matrix[snp.row,]) / n.samples
```
...
Create an expression matrix contaiing transcripts-per-million (TPM) as a measure of gene expression. Could alternatively use RPKM
...
# scale RPK values to get TPM
all.tpm.matrix <- all.rpk.matrix / (rpk.sum/1e6)
```
...
Next perform linear regression of the genoypes of each SNP against the expression of the gene and 
