...
##### Answer
There could be lots of ways to do this.  Here is one
```{r}
primeNumbers=2
for (i in 2:500){
  isPrime=TRUE
  for (j in 1:length(primeNumbers)){#strictly, only really have to go up through sqrt(i)
    if (i/primeNumbers[j]==floor(i/primeNumbers[j])){
      isPrime=FALSE
      #could put a break here
    }
  }
  if (isPrime==TRUE){
...
```{r}
yfp=tbl_df(read.csv('http://labs.biology.ucsd.edu/rifkin/courses/BIOM262/Stats/W2017/timeDataWide_YFP.csv',stringsAsFactors=FALSE))
cfp=tbl_df(read.csv('http://labs.biology.ucsd.edu/rifkin/courses/BIOM262/Stats/W2017/timeDataWide_CFP.csv',stringsAsFactors=FALSE))
```
...
```{r}
# Some preprocessing
yfp2=gather(yfp,frame,yfpLevel,3:134,na.rm=T) %>% separate(frame,into=c("ff","frame")) %>% select(-ff)
cfp2=gather(cfp,frame,cfpLevel,3:134,na.rm=T) %>% separate(frame,into=c("ff","frame")) %>% select(-ff)
both=left_join(yfp2,cfp2,by=c('cellID','cellType','frame'))
both$cellID=as.factor(both$cellID)
both$cellType=as.factor(both$cellType)
both$frame=as.numeric(both$frame)
```
...
# to do the facetting cfp and yfp need to be values of a variable instead of variables themselves
both2=gather(both,fluor,lev,cfpLevel,yfpLevel) 

ggplot(both2)+geom_line(aes(x=frame,y=lev,group=cellID,color=cellType))+facet_grid(fluor~.)
...
ggplot(both2)+geom_line(aes(x=frame,y=lev,group=cellID,color=cellType),alpha=.1)+facet_grid(fluor~.)
...
persistence=count(both,cellID) %>% arrange(n)

cellsPresentFor10OrMoreFrames=persistence$cellID[persistence$n>=10] 
# another way:
cellsPresentFor10OrMoreFrames=filter(persistence,n>=10) %>% select(cellID)

ggplot(persistence)+geom_point(aes(x=cellID,y=n))
...
persistence$cellID =factor(persistence$cellID, levels=persistence$cellID)
ggplot(persistence)+geom_point(aes(x=cellID,y=n))
...
ggplot(persistence)+geom_histogram(aes(x=n))
ggplot(persistence)+geom_density(aes(x=n))
...
```{r}
#code for 2.3
g60=persistence$cellID[persistence$n>=60]
both60=filter(both,cellID %in% g60, cellType=='expt')
cellIDsToUse=unique(as.character(both60$cellID)) %>% sample(5,replace=F)

both60.2=filter(both60,cellID %in% cellIDsToUse)
ggplot(both60.2)+geom_line(aes(x=cfpLevel,y=yfpLevel,color=frame,group=cellID))
```
...
cellsToPlot=filter(both2,cellID %in% cellIDsToUse)
ggplot(cellsToPlot)+geom_line(aes(x=frame,y=lev,group=cellID,color=cellID))+facet_grid(fluor~.)
